...
class PrologTokenizer {
	...
	boolean splitoff() {
		...
		/*
		 * step 1: skip whitespace 2: decide on first character what the token
		 * kind is. 3: seek the end of the token (start) 4: shorten unused, add
		 * the token, return true;
		 */
		if (unused == null)
			return false;
		...
		if (c == 39 || c == '"') {
			...
			for (; start < max; start++) {
				...
				if (d == c) {
					start++;
					if (start < max && unused.charAt(start) == c)
						;// mind the ;
					else {
						closed = true;
						break;
