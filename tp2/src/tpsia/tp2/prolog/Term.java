...
class Term {
	...
	{
		if (inp.length() != 0 && PrologTokenizer.in(inp.charAt(0), normalstart)) {
	...
	static boolean match(Term in1, Term in2, Stack substitutions) {
		/*
		 * match to variables. all variables that are instantiated are added to
		 * substitutions.
		 */
		Stack s = new Stack();
		Stack t = new Stack();
		...
		/*
		 * Instead of stacking pairs I have a pair of stacks. Both stacks will
	...
	static int numbervalue(Term t) {
		/*
		 * calculate the number represented by a term. the term must consist of
		 * NUMBER constants kept together by +,-,*,/,mod.
		 */
		t = skipeq(t);
		if (t.type == NUMBER)
			return t.arity;
	...
	static String[] conso1 = { "B", "D", "F", "G", "H", "K", "L", "M", "N",
			"Z", "X" };
	...
	static String ebbinghaus() {
		...
		count++;
		return conso1[count % conso1.length] + vowel[count % vowel.length]
	...
	static String toString(Term t, int level, boolean q) {
		...
		case FUNCTOR:
			if (t.name == Prologop.listcons.name)/* try list display */
				return "[" + toString(t.arg[0], 999, q)
						+ tailstring(t.arg[1], q);
			if (t.arity == 0)
				if (q)
					return t.qname;
				else
	...
	static Term parset(PrologTokenizer tok, Hashtable vars, int level) {
		/*
		 * parse a tokenized string to a term, with operatorlevel<=level The
		 * hashtable contains the variables that are allready made.
		 */
		if (!tok.more())
			return null;
		...
		if (p1 != null && p1.priority <= level) {
			...
			if (t1 != null) {
				t1 = readfurther(new Term(p1, t1), p1.priority, tok, vars,
						level);
				if (t1 != null)
					return t1;
			}
	...
	static Term readfurther(Term t1, int t1level, PrologTokenizer tok,
			Hashtable vars, int level) {
		...
		if (p1 != null && p1.priority <= level && t1level < p1.leftunderlevel()) {
			t = readfurther(new Term(p1, t1), p1.priority, tok, vars, level);
			if (t != null)
				return t;
		}
		tok.jumpto(tokpos);
	...
	static Term fixin(Term t1, Prologop o1, Term t2, PrologTokenizer tok,
			Hashtable vars, int highlevel) {
		if (!tok.more())
		...
		if (o2 != null && o2.priority <= highlevel) {
			...
			if (t3 != null) {
				if (o1.under(o1, o2) == 1) {
					t = fixin(new Term(t1, o1, t2), o2, t3, tok, vars,
							highlevel);
					if (t != null)
						return t;
	...
	static Term parsetbasic(PrologTokenizer tok, Hashtable vars) {
		...
		if (PrologTokenizer.in(first, varstart)) {
			...
			int n;
			try {
				n = Integer.parseInt(tok.gettoken());
