...
public class CalculadorTrucho implements Cloneable{
	 ...
	 */
	public CalculadorTrucho(String grupo){
		super();
		inicializar(grupo);
	}
	/**
	...
	private void inicializar(String grupo){
	      ...
	      energiaPacman = 30; //new Double(Math.random()* 30 + 20).intValue();
	      energiaPacmanOriginal = energiaPacman;
	      energiaEnemigo = 30 ; //new Double(Math.random()* 45 + 5).intValue();
	      energiaAlimentos = 10; //new Double(Math.random()* 20 + 5).intValue();
	      ...
	      for (int i=0;i<posiciones.length;i++)
	          ...
	          {
	        	  posiciones[i][j]=CalculadorTrucho.EMPTY;
	        	  posicionesInicial[i][j]=CalculadorTrucho.EMPTY;
	          }
	      ...
	      x = 1; //new Double(Math.random()* 4+1).intValue();
		  y = 3; //new Double(Math.random()* 4+1).intValue();
		  xIni = x;
		  yIni = y;
	...
	private void generarPosicionesEnemigo(){
		int tamanioVector = 2; //new Double(Math.random()*4 + 2).intValue();
		Vector salida = new Vector(tamanioVector);	
		Pair pos;
		int x,y;
		int[] ene = { 1 , 3 , 4, 4};
		for (int i=0; i< tamanioVector; i=i+2){
			do{
			 x = ene[i+1];//new Double(Math.random()* 4+1).intValue();
			 y = ene[i+2]; //new Double(Math.random()* 4+1).intValue();
		  	 pos = new Pair(x,y);
		  	 }while(repetido((Pair)pos));
		     //System.out.println("-- Pos Enemigo: " + x + "," + y);
			 salida.add(pos);
			 enemigos.add(pos);
			 posiciones[x-1][y-1]=CalculadorTrucho.ENEMY;
			 posicionesInicial[x-1][y-1]=CalculadorTrucho.ENEMY;
		  	}
	}
	...
	private void generarPosicionesComida(){
		int tamanioVector = 2; //new Double(Math.random()*6 + 2).intValue();
		Vector salida = new Vector(tamanioVector);	
		Pair pos;
		int x,y;
		int[] ene = { 3 , 3 , 4, 2};
		for (int i=0; i< tamanioVector; i=i+2){
			...
			salida.add(pos);
			comida.add(pos);
			posiciones[x-1][y-1]=CalculadorTrucho.FOOD;
			posicionesInicial[x-1][y-1]=CalculadorTrucho.FOOD;
		  	}
	}
	...
	public int calcularEnergiaPacMan(String a){
		if (energiaPacman > 0){
			if (a.toLowerCase()=="arriba" | a.toLowerCase()=="abajo" | a.toLowerCase()=="izquierda" | a.toLowerCase()=="derecha"){
				...
				if (a.toLowerCase() == "pelear" & posiciones[x-1][y-1]==CalculadorTrucho.ENEMY)
				{
					posiciones[x-1][y-1] = CalculadorTrucho.EMPTY;
					...
					if (a.toLowerCase() == "comer" & posiciones[x-1][y-1]==CalculadorTrucho.FOOD){
						posiciones[x-1][y-1] = CalculadorTrucho.EMPTY;
	...
	public CalculadorTrucho clone() {
		CalculadorTrucho c = null;
		try {
			c = (CalculadorTrucho) super.clone();
